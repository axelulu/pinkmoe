...
RUN export NODE_OPTIONS=--max-old-space-size=4096 && cd ./pinkmoe_admin && npm install npm@latest -g && npm i && npm run build
...
RUN export NODE_OPTIONS=--max-old-space-size=4096 && cd /pinkmoe_index && npm install npm@latest -g && npm install pnpm -g && pnpm i && pnpm build
