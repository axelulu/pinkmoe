    ...
    <div class="ml-6">
      ...
      >
        ...
        <div class="flex flex-wrap items-center px-4 pb-4">
          <Spin :show="loading">
            <div class="w-full" v-if="authorRecordList?.list && authorRecordList?.list?.length">
              <div
              ...
              >
                <div v-if="item.type === 'publishPost'" class="flex justify-between items-center">
                  <div class="flex flex-row justify-center items-center">
                    ...
                    <font-awesome-icon class="text-xl px-4" :icon="['fas', 'newspaper']" />
                    <div
                      >您的文章<router-link
                        class="text-pink-400"
                        :to="'/post/' + item?.postRelation?.postId"
                        >《{{ item?.postRelation?.title }}》</router-link
                      >审核成功！</div
                    >
                  </div>
                  <div class="min-w-16 text-right">{{ formatDate(item?.UpdatedAt) }}</div>
                </div>
                <div
                ...
                >
                  <div class="flex flex-row justify-center items-center">
                    ...
                    <font-awesome-icon class="text-xl px-4" :icon="['fas', 'download']" />
                    <div
                ...
                <div v-else-if="item.type === 'buyVideo'" class="flex justify-between items-center">
                  <div class="flex flex-row justify-center items-center">
                    ...
                    <font-awesome-icon class="text-xl px-4" :icon="['fas', 'video']" />
                    <div
                      >恭喜您成功购买了文章<router-link
                        class="text-pink-400"
                        :to="'/post/' + item?.postRelation?.postId"
                ...
                <div v-else-if="item.type === 'buyMusic'" class="flex justify-between items-center">
                  <div class="flex flex-row justify-center items-center">
                    ...
                    <font-awesome-icon class="text-xl px-4" :icon="['fas', 'music']" />
                    <div
                      >恭喜您成功购买了文章<router-link
                        class="text-pink-400"
                        :to="'/post/' + item?.postRelation?.postId"
                ...
                >
                  <div class="flex flex-row justify-center items-center">
                    ...
                    <font-awesome-icon class="text-xl px-4" :icon="['fas', 'comments']" />
                    <div
                      >您对用户<router-link
                        class="text-pink-400"
                        :to="'/author/' + item?.userRelation?.uuid + '/userInfo'"
                        >{{ item?.userRelation?.nickName }}</router-link
                      >在文章<router-link
                        class="text-pink-400"
                        :to="'/post/' + item?.postRelation?.postId"
                        >《{{ item?.postRelation?.title }}》</router-link
