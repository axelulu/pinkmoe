 ...
 * @LastEditTime: 2022-08-07 09:06:19
 * @FilePath: /pinkmoe_index/src/pages/user-center/vip/index.vue
...
<script lang="ts" setup name="UserCenterVipIndex">
  ...
  const { authorAuthorityList, auth, currentIndex, buyVip } = useUserCenterVip();
  useHead({
    title: `会员 - 用户中心`,
    meta: [
    ...
    <div class="ml-6">
      ...
      >
        ...
        <div class="flex flex-col items-center px-4 pb-4">
          ...
          >
            <p class="text-xs py-1.5"
              ...
              ><span class="ml-4 bg-green-400 rounded-xl text-white px-1 py-0.5"
                ><font-awesome-icon :icon="['fas', 'paw']" /> 积分{{ auth.userInfo?.credit }}</span
              ></p
            >
            <p class="text-xs py-1.5"
          ...
          <div class="w-full py-4 flex flex-wrap justify-center">
            <div
            ...
            >
              ...
              >
                <div class="text-center w-full p-2">
                  <div class="text-base pb-2 pt-2 text-blue-400">{{ authority.authorityName }}</div>
                  <div v-for="(k, v) in authority.authorityParams" :key="v">
                    ...
                    >
                      <font-awesome-icon class="mr-2 text-green-500" :icon="['fas', 'check']" />{{
                        k.authorityParamKey
                      }}{{ k.authorityParamValue }}%</div
                    >
                    <div v-else :class="`text-base py-0.5 text-gray-800 dark:text-gray-200`"
                      >￥{{ k.authorityParamValue }}/
                      <span class="text-gray-400">{{ k.authorityParamKey }}</span></div
                    >
                  </div>
                </div>
              </div>
