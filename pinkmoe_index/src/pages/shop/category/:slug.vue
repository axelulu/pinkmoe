 ...
 * @LastEditTime: 2022-08-20 21:32:25
 * @FilePath: /pinkmoe_vitesse/src/pages/shop/category/:slug.vue
...
import { useShopCategory } from '/@/hooks/shop/category'

const { loading, hasMore, route, title, categoryPostList, sort, sortPost, nextPage, formParams }
    = useShopCategory()

const { siteBasic } = useAppStore()
useHead({
  title: `${computed(() => title.value)} - 商品分类页面`,
  meta: [
  ...
  <div class="flex flex-col items-center min-h-86">
    <div class="lg:w-3/4 xl:w-5/12 text-gray-500">
      ...
      >
        ...
        >
          <i class="inline-block text-xs py-3 px-2 ml-1 i-fluent:caret-right-12-filled" />
          <span
      ...
      <Spin :show="loading">
        <div
          v-if="categoryPostList?.list?.goods?.length"
          class="w-full flex justify-start flex-wrap mt-4 animate-fadeIn30"
        >
          <div v-for="(post, v) in categoryPostList.list.goods" :key="v" class="w-1/6 p-1.5">
