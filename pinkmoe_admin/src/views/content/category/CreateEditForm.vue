 ...
 * @LastEditTime: 2022-08-21 16:43:00
 * @FilePath: /pinkmoe_admin/src/views/content/category/CreateEditForm.vue
...
<script lang="ts">
  ...
  import { createCategory, updateCategory, getCategoryList } from '@/api/content/category';
  ...
  const rules = {
    ...
    icon: {
      ...
      trigger: 'blur',
      message: '请输入2到100位（字母）',
      pattern: /^[a-zA-Z1-9:_-]{2,100}$/, //验证图标 2到30位（字母）
    },
    parentId: {
      required: true,
      trigger: 'blur',
          ...
          if (!errors) {
            const { code, message: msg } =
              props.formType === 'create'
                ? await createCategory(state.formParams)
                : await updateCategory(state.formParams);
            message.destroyAll();
            if (code == ResultEnum.SUCCESS) {
