 ...
 * @LastEditTime: 2022-08-16 20:05:49
 * @FilePath: /pinkmoe_admin/src/views/shop/category/category.vue
...
import { getShopCategoryList, deleteShopCategory } from "@/api/shop/category";
...
const loadDataTable = async (res) => {
  const {
    ...
    message: msg,
    result
  } = await getShopCategoryList({ ...formParams, ...params.value, ...res });
...
function handleDelete(record: Recordable) {
  dialog.info({
    ...
    onPositiveClick: async () => {
      const { code, message: msg } = await deleteShopCategory(record);
      message.destroyAll();
      if (code == ResultEnum.SUCCESS) {
